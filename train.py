...
def split_and_tokenize_data(tokenizer, max_length = config["max_length"]):
    ...
    X_train = np.array([tokenizer.encode(x, max_length = max_length, pad_to_max_length = True, truncation=True) for x in train_df.sentence])
    X_test = np.array([tokenizer.encode(x, max_length = max_length, pad_to_max_length = True, truncation=True) for x in test_df.sentence])

    y_train = y_train = np.array(train_df.label)
...
X_train, y_train, X_test, y_test = split_and_tokenize_data(tokenizer)
